-- Full-text search over corpus.provisions with ranking and headlines.
--
-- Uses the generated tsvector column `fts` (indexed via GIN at
-- idx_provisions_fts) and `websearch_to_tsquery` so the query string can
...
--
-- Returns one row per hit with the canonical metadata the Axiom viewer
...
-- security on corpus.provisions still applies.
GRANT EXECUTE ON FUNCTION corpus.search_provisions(text, text, text, int) TO anon;
GRANT EXECUTE ON FUNCTION corpus.search_provisions(text, text, text, int) TO authenticated;
