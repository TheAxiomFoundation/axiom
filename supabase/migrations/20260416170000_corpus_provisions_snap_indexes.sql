-- Indexes that unblock Axiom title- and part-level browsing.
--
-- The viewer renders a rule's children by looking up all `corpus.provisions` rows
...
--     SELECT * FROM corpus.provisions
--      WHERE citation_path LIKE 'us/statute/7/%'
--        AND parent_id IS NULL
--      ORDER BY ordinal
--      LIMIT 100;
-- degenerates into a scan of ~65k rows on the existing partial index
-- `idx_provisions_toplevel_sort (jurisdiction, source_path) WHERE parent_id IS NULL`
...
--   EXPLAIN ANALYZE SELECT * FROM corpus.provisions
...
--           ->  Index Scan using idx_provisions_orphan_citation_prefix
...
--   EXPLAIN ANALYZE SELECT COUNT(*) FROM corpus.provisions
--    WHERE citation_path LIKE 'us/statute/7/%' AND parent_id IS NULL;
--   Aggregate  (actual 1.785..1.785)
--     ->  Index Only Scan using idx_provisions_orphan_citation_prefix
...
CREATE INDEX IF NOT EXISTS idx_provisions_orphan_citation_prefix_ordinal
  ON corpus.provisions (citation_path text_pattern_ops, ordinal)
...
-- doesn't matter.
CREATE INDEX IF NOT EXISTS idx_provisions_orphan_citation_prefix
  ON corpus.provisions (citation_path text_pattern_ops)
  WHERE parent_id IS NULL;
