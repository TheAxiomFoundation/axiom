...
Usage:
    uv run python -m axiom.crawl_playwright us-al --max-sections 100
    uv run python -m axiom.crawl_playwright --all --dry-run
"""
...
from axiom.crawl import R2_BUCKET, get_r2_client
...
def main(
    ...
    Examples:
        ...
        uv run python -m axiom.crawl_playwright us-al --max-sections 10 --headed

        # Crawl all SPA states
        uv run python -m axiom.crawl_playwright --all

        # Dry run Texas
        uv run python -m axiom.crawl_playwright us-tx --dry-run
    """
    headless = not headed
    ...
    if crawl_all:
        ...
        click.echo("\nUsage: uv run python -m axiom.crawl_playwright us-al")
