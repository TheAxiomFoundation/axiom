...
Environment variable: AXIOM_SUPABASE_DB_URL
"""
...
def get_db_url() -> str:
    ...
    url = os.environ.get("AXIOM_SUPABASE_DB_URL")
    if not url:
        raise ValueError(
            "AXIOM_SUPABASE_DB_URL environment variable not set. "
...
class SupabaseStorage:
    ...
    Schema:
    - corpus.statutes: Main statute storage with tsvector for FTS
    - corpus.cross_references: Citation cross-reference graph
    - corpus.jurisdictions: Jurisdiction metadata
    """
    ...
    def __init__(self, db_url: str | None = None):
        ...
        Args:
            db_url: PostgreSQL connection URL. If None, reads from
                    AXIOM_SUPABASE_DB_URL environment variable.
        """
        self._conn = None
        if not HAS_PSYCOPG:
            raise ImportError(
    ...
    def _init_schema(self):
        ...
        with self.conn.cursor() as cur:
            ...
            # Main statutes table
            cur.execute("""
                CREATE TABLE IF NOT EXISTS corpus.statutes (
            ...
            cur.execute("""
                ...
                ON corpus.statutes USING GIN(fts_vector)
            """)

            # Indexes for common queries
            cur.execute("""
                CREATE INDEX IF NOT EXISTS idx_statutes_jurisdiction
                ON corpus.statutes(jurisdiction)
            """)
            cur.execute("""
                CREATE INDEX IF NOT EXISTS idx_statutes_code
                ON corpus.statutes(jurisdiction, code)
            """)

            # Trigger to update FTS vector
            cur.execute("""
                CREATE OR REPLACE FUNCTION corpus.update_fts_vector()
                RETURNS TRIGGER AS $$
                BEGIN
            ...
            """)
            cur.execute("""
                DROP TRIGGER IF EXISTS trg_statutes_fts ON corpus.statutes
            """)
            cur.execute("""
                CREATE TRIGGER trg_statutes_fts
                BEFORE INSERT OR UPDATE ON corpus.statutes
                FOR EACH ROW EXECUTE FUNCTION corpus.update_fts_vector()
            """)

            # Cross-references table
            cur.execute("""
                CREATE TABLE IF NOT EXISTS corpus.cross_references (
            ...
            cur.execute("""
                CREATE TABLE IF NOT EXISTS corpus.jurisdictions (
        ...
        with self.conn.cursor() as cur:
            cur.execute("""
                SELECT
                    ...
                    j.last_updated
                FROM corpus.jurisdictions j
                ORDER BY j.jurisdiction
            """)
            rows = cur.fetchall()

        from axiom_corpus.models_statute import JurisdictionType
    ...
    def update_jurisdiction_stats(self, jurisdiction: str) -> None:
        ...
        with self.conn.cursor() as cur:
            cur.execute(
                """
                UPDATE corpus.jurisdictions
                SET
                    section_count = (
                        SELECT COUNT(*) FROM corpus.statutes
    ...
    def get_section_count(self, jurisdiction: str | None = None) -> int:
        ...
        with self.conn.cursor() as cur:
            if jurisdiction:
                cur.execute(
                    "SELECT COUNT(*) as count FROM corpus.statutes WHERE jurisdiction = %s",
                    [jurisdiction],
                )
            else:
                cur.execute("SELECT COUNT(*) as count FROM corpus.statutes")
